...
function handleMouse() { // swings and draws the sword
...
function handleFruit() { // pushes and updates fruit, returns number of points scored
  if (frameCount % 10 === 0) { // push new fruit
  ...
  var points = 0;
  for (var i = fruit.length - 1; i >= 0; i--) { // slice fruit
   fruit[i].update();
   fruit[i].draw();
